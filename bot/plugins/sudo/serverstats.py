...
async def stats(_, message: Message):
    image = Image.open('bot/helpers/assets/statsbg.jpg').convert('RGB')
    ironfont = ImageFont.truetype("bot/helpers/assets/ironfont.otf", 42)
    ...
    osuptime = get_readable_time(int(time.time() - psutil.boot_time()))
    botusage = f"{round(process.memory_info()[0] / 1024 ** 2)} MiB"

    upload = get_readable_bytes(str(psutil.net_io_counters().bytes_sent))
    download = get_readable_bytes(str(psutil.net_io_counters().bytes_recv))

    cpu_percentage = psutil.cpu_percent()
    cpu_count = psutil.cpu_count()

    ram_percentage = psutil.virtual_memory().percent
    ram_total = get_readable_bytes(str(psutil.virtual_memory().total))
    ram_used = get_readable_bytes(str(psutil.virtual_memory().used))
    ...
    start = datetime.now()
    msg = await message.reply_photo(photo="https://te.legra.ph/file/30a82c22854971d0232c7.jpg", caption=caption,
                                    quote=True)
    end = datetime.now()
